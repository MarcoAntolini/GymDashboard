...
export async function POST(req: NextRequest) {
	try {
		...
		if (!isAppRole(account.role)) {
			return NextResponse.json({ message: "Invalid account role", success: false }, { status: 500 });
		}

		const now = Math.floor(Date.now() / 1000);
		const { payloadB64, payload } = createSessionValue(username, account.role, now);
		const value = await signSessionValue(payloadB64);
		const redirectTo = landingPathForRole(account.role);

		const res = NextResponse.json(
			{ message: "Logged in", success: true, role: account.role, redirectTo },
			{ status: 200 }
		);
