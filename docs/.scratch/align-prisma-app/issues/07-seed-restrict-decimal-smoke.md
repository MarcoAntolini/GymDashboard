...
- [x] Orchestrazione seed: Acquisti prima degli Ingressi; Ingressi solo con `purchaseId` valido (niente mock basato solo su `clientId`)
- [x] Smoke: Acquisto → Ingresso ok; residuo esaurito → reject
- [x] Delete Prodotto con Acquisti / Acquisto con Ingressi / Cliente con Acquisti → errori Restrict utente-facing
- [x] Form/display denaro (Acquisti, Listino, Contratti/Pagamenti toccati) non rompono con `Decimal` Prisma
- [x] App smoke base verde sul percorso critico (tsc o avvio + flusso ingresso)
