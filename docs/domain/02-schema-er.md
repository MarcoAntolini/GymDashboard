...
```mermaid
erDiagram
	CLIENTE ||--o{ VENDITA : effettua
	PRODOTTO ||--o{ VENDITA : oggetto
	VENDITA ||--o{ INGRESSO : giustifica
...
Niente contatore di ingressi sul Cliente: il residuo da pacchetto è derivabile dalle Vendite di Pacchetto e dagli Ingressi che le giustificano (vedi regole e `03-schema-logico.md`).
...
Anagrafiche simili a CLIENTE, ma **senza** generalizzazione PERSONA in questo schema: cicli di vita e associazioni restano disgiunti (iscrizione/vendite/ingressi vs assunzione/contratti/timbrature/account). La stessa persona fisica può essere sia cliente sia dipendente come due istanze distinte — con PERSONA (t,e) sarebbe vietato.
...
Solo `Codice` (PK). Specializzazioni sotto. Il tipo (Abbonamento vs Pacchetto) è determinato dalla specializzazione, non da un attributo ridondante su Listino/Vendita.
...
Pattern visite/reificazione (`03-modello-er.md`): Vendita è entità evento, non sola associazione N:M Cliente–Prodotto.
...
Reifica l’accesso in palestra (eventi ripetibili). Il Cliente si raggiunge navigando la Vendita che giustifica l’ingresso — niente associazione diretta Cliente–Ingresso (eviterebbe un ciclo ridondante con `Vendita.Cliente`).
...
| Contatore residuo su Vendita | **No** | Residuo = `N − COUNT`; denormalizzazione solo se misure di carico lo richiedono |
| `TitoloAccesso` distinto da Vendita | **No** | Coinciderebbe ~1:1 con Vendita |
...
| EFFETTUA_VENDITA | CLIENTE | (0,N) | VENDITA | (1,1) | Data, Importo |
| OGGETTO_VENDITA | PRODOTTO | (0,N) | VENDITA | (1,1) | — |
| GIUSTIFICA | VENDITA | (0,N) | INGRESSO | (1,1) | Data (su Ingresso) |
...
- Un Cliente **può** non avere ancora vendite; gli ingressi esistono solo se giustificati da una Vendita.
- Un Prodotto **può** non avere ancora righe di listino o vendite.
- Una Vendita **può** non avere ancora ingressi collegati (abbonamento appena comprato, pacchetto non ancora usato).
- Preferenza alle binarie rispetto a una ternaria Cliente–Vendita–Ingresso: ogni Ingresso determina esattamente una Vendita (max-card 1) → falsa ternaria (`03-modello-er.md`).
...
| VENDITA | Id | surrogato; non (Cliente, Data) |
| INGRESSO | Id | surrogato; FK obbligatoria ad Vendita |
| PAGAMENTO | Id | |
...
3. Il tipo di una Vendita/Listino è quello della specializzazione del Prodotto collegato (niente attributo `Tipo` ridondante).
...
8. Ogni Ingresso **deve** riferirsi a una Vendita dello stesso Cliente (via `Vendita.Cliente`) che giustifica l’accesso all’istante `Ingresso.Data`.
9. Scelta della Vendita giustificatrice (**una transazione**, vedi `03-schema-logico.md`):
	- candidati Abbonamento: Vendite di Abbonamento del Cliente con `Ingresso.Data ∈ [DataVendita, DataVendita + DurataSnapshot)`;
	- se ce n’è almeno uno → scegliere quello con `DataVendita` **più recente**; in parità, `Id` maggiore (nessun consumo pacchetto);
	- altrimenti candidati Pacchetto: Vendite di Pacchetto con `residuo = NumeroIngressiSnapshot − COUNT > 0`; scegliere quello con `DataVendita` **meno recente** (FIFO); in parità, `Id` minore;
...
12. `Vendita.Importo` è lo **snapshot** dell’importo alla vendita (storico cassa). Di default coincide con `Listino.Prezzo` del Prodotto per `anno = YEAR(DataVendita)`; può differire solo per sconto/deroga esplicita in operazione. Non è una FK al Listino: il listino può cambiare in seguito senza riscrivere la storia.
13. Cancellazioni che preservano la storia economica (**RESTRICT**):
	- Prodotto con Vendite esistenti;
	- Cliente con Vendite esistenti;
	- Vendita con Ingressi esistenti.
...
| Minimalità | Nessun `TitoloAccesso` (~1:1 con Vendita); nessun contatore globale su Cliente |
